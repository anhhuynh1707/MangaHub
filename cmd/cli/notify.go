...
func handleNotify(args []string) {
	if len(args) == 0 {
		fmt.Println("Usage: mangahub notify <subscribe|unsubscribe|test|send>")
		fmt.Println()
		...
		fmt.Println("  send          Send a test notification to all subscribers (via HTTP API)")
		return
	}
	...
	default:
		...
		fmt.Println("Available: subscribe, unsubscribe, test, send")
	}
}
