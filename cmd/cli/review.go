...
func handleReview(args []string) {
	if len(args) == 0 {
		fmt.Println("Usage: mangahub review <add|list|mine>")
		return
	}
	...
	default:
		...
		fmt.Println("Available: add, list, mine")
	}
}
...
func reviewList(args []string) {
	...
	for _, rev := range data.Reviews {
		fmt.Printf("[%s] %s - Rating: %d/10 (Helpful: %d)\n", rev.CreatedAt, rev.Username, rev.Rating, rev.Helpful)
		if rev.Text != "" {
			fmt.Printf("  \"%s\"\n", rev.Text)
		}
		fmt.Println()
